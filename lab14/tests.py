...
class CiNsuNewsPage_Test:
	...
	def __init__(self):
		...
		self.page.offset_date_after(datetime(2022, 12, 1, tzinfo=timezone.utc))
		self.page.offset_date_before(datetime(2024, 12, 1, tzinfo=timezone.utc))
		time.sleep(2)
	...
	@pytest.mark.parametrize("first", "last", [{datetime(2022, 12, 1, tzinfo=timezone.utc), datetime(2024, 12, 1, tzinfo=timezone.utc)}])
	def verify_first_and_last_news_dates(self, first_value, second_value):
		...
		last_date = datetime.strptime(datelist[len(datelist) - 1].text, "%d.%m.%Y").replace(tzinfo=timezone.utc)
